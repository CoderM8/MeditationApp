...
class DatabaseHelper {
  ...
  Future<bool> isdownload(String id) async {
    ...
    final List<Map<String, dynamic>> maps =
        await db!.query('DownloadSong', where: 'id = ?', whereArgs: [id]);
    return maps.isNotEmpty ? true : false;
  }

  Future addDownload(DownloadSong dSongs) async {
    var db = await database;
    dSongs.id = await db!.insert('DownloadSong', dSongs.toMap(),
  ...
  Future<List<DownloadSong>> getDownloadSongs() async {
    var db = await database;
    List<Map> maps = await db!.query('DownloadSong',
        /*where: 'userId = ?',*/ /*whereArgs: [userId]*/ columns: [
          'id',
          'songid',
          'artist',
          'link',
