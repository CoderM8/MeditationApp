...
class EnergizeBreathCircle extends StatefulWidget {
  final String? time;
  final Duration? duration;
  EnergizeBreathCircle({Key? key, this.time, this.duration}) : super(key: key);

  @override
  State<EnergizeBreathCircle> createState() => _EnergizeBreathCircleState();
}

class _EnergizeBreathCircleState extends State<EnergizeBreathCircle>
  ...
  void startBreathingCycle() {
    setState(() {
      displayText =
          inhaleExhale.value ? 'Inhale 4 Seconds' : 'Exhale 2 Seconds';
      iconPath = inhaleExhale.value
          ? 'assets/icons/relax1.svg'
          : 'assets/icons/relax2.svg';
    });

    _timer = Timer(Duration(seconds: inhaleExhale.value ? 4 : 2), () {
      setState(() {
        inhaleExhale.value =
